...
public class AppointmentController {
    ...
    @PutMapping("/{id}/complete") @PreAuthorize("hasRole('DOCTOR')")
    public ResponseEntity<GeneralResponse> complete(@PathVariable Long id,
                                                    @AuthenticationPrincipal User user) {
        return ResponseEntity.ok(GeneralResponse.builder()
                .data(appointmentService.complete(id, user.getEmail())).message("Cita completada").build());
    }
